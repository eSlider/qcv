...
CUInt2DParameterEditor::CUInt2DParameterEditor( CUInt2DParameter *  f_parameter_p,
                                                QWidget *          f_parent_p )
...
{    
    ...
    for (int i = 0; i < 2; ++i)
    {
        m_qsb_p[i] = new CUIntEditor( 0, this );
    ...
    connect( m_qsb_p[0],  SIGNAL( valueChanged ( unsigned int ) ),
             this,        SLOT(   updateParameter() ) );

    connect( m_qsb_p[1],  SIGNAL( valueChanged ( unsigned int ) ),
             this,        SLOT(   updateParameter() ) );
}

CUInt2DParameterEditor::~CUInt2DParameterEditor ()
{}

void
CUInt2DParameterEditor::updateParameter()
{
    S2D<unsigned int> value( m_qsb_p[0]->getValue(),
                             m_qsb_p[1]->getValue() );
...
bool
CUInt2DParameterEditor::updateWidget()
{
    S2D<unsigned int> value = m_parameter_p -> getValue();
